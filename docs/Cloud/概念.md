...
GitOps 的部署策略有两种实现方式：推送式部署和拉取式部署。如果条件允许，应优先选择拉取式部署，因为它被认为更安全，因此也是实现 GitOps 的更佳实践。

推送式感觉类似 GitHub Actions，靠 Jenkins 等工具，在 push 后，跑流水线、构建，并推送。此时，该流水线拥有完全的权限。

拉取式：则更类似于 Argo CD 的操作，环境只有在环境仓库更新时才会更新
