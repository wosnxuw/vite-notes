...
node 节点（如果机器翻译翻译成节点的东西，一般就是指 node，pod 很多时候机翻会跳过不翻译）
...
所以你需要 `kubectl apply -f xxx.yaml` yaml 描述了一个 Service 资源；kind: Service 告诉 Kubernetes 这是哪类资源；Kubernetes API Server 会按标准解析、校验和保存它，然后相关控制器会把它转成实际可用的服务转发规则
