...
| 连接两个进程，左边 stdout 做右边 stdin

`>` 左边是命令，命令的 stdout，右边是文件，覆盖/新建

`>>` 和 `>` 唯一区别是追加

如果你要写入内容，你可以 `echo hello > file` `echo hello >> file`
 
必须有 echo，否则你没有进程的 stdout

从 `cat > file.txt << abcd` 来思考

启动 cat，并设置 cat 的 stdout 是 file.txt，stdin 是 heredoc，即那个字符串

`<< abcd` 属于 heredoc，直到再次遇到一行单独的 abcd，一般是用 EOF，只是一个标记，并不是真正的关键字

1. 收集 heredoc 内容
2. 打开 file
3. 创建子进程
4. 把子进程 fd 0 接到 heredoc
5. 把子进程 fd 1 接到 file
6. exec cat

所以 `cat << abcd > file` 是一样的

虽然看起来 cat 像是一个管道，但是它不能放在中间，否则 shell 不知道启动什么程序
